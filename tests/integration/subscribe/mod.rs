...
async fn subscribe_recv_unsubscribe() {
    ...
    assert_eq!(
        message_arg_0.get("a").unwrap().clone().into_int().unwrap(),
        1
    );
    assert_eq!(
        message_arg_0.get("b").unwrap().clone().into_bool().unwrap(),
        false
    );
    let list = message_arg_0.get("c").unwrap().clone().into_list().unwrap();
    assert_eq!(list.len(), 3);
    assert_eq!(list[0].clone().into_int().unwrap(), 3);
    assert_eq!(list[1].clone().into_int().unwrap(), 4);
    assert_eq!(list[2].clone().into_int().unwrap(), 5);
